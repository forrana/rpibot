            ...
            if is_rpi:
                ...
                for target in targets_to_try:
                    try:
                        ...
                        # Try different configuration approaches for libcamera compatibility
                        config = None
                        try:
                            # Try the most basic configuration first (compatible with older libcamera)
                            config = test_camera.create_video_configuration()
                            # Use simple configure without any optional parameters
                            test_camera.configure(config)
                        except Exception as config_error:
                            print(f"Basic configuration failed: {config_error}, trying fallback...")
                            try:
                                # Try creating configuration with minimal parameters
                                config = test_camera.create_video_configuration(
                                    main={"size": (640, 480)}
                                )
                                test_camera.configure(config)
                            except Exception as config_error2:
                                print(f"Minimal configuration failed: {config_error2}, trying direct start...")
                                try:
                                    # Try to start without explicit configuration
                                    test_camera.start()
                                    # If we get here, it worked - stop and continue
                                    test_camera.stop()
                                except Exception as config_error3:
                                    print(f"Direct start also failed: {config_error3}")
